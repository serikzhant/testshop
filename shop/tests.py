...
class ProductViewTest(TestCase):
    def test_get_products(self):
        ...
        uploaded = SimpleUploadedFile(
            'test_image_gif', small_gif, content_type='image/gif'
        )
        category = Category.objects.create(name="test")
        product_1 = Product.objects.create(
            title='Product 1', category=category, image=uploaded, price=100, slug='product-1', available=True
        )
        product_2 = Product.objects.create(
            title='Product 2', category=category, image=uploaded, price=100, slug='product-2', available=True
        )
        ...
        self.assertEqual(
            list(response.context['products']), [product_1, product_2])
...
class ProductDetailViewTest(TestCase):
    def test_get_product_by_slug(self):
        ...
        product = Product.objects.create(
            title='Product 1', category=category, slug='product-1', image=uploaded, price=100, available=True)
...
class CategoryListViewTest(TestCase):
    def setUp(self):
        ...
        self.product = ProductProxy.objects.create(
            title='Test Product', slug='test-product', category=self.category, image=uploaded, price=100, available=True)
    ...
    def test_template_used(self):
        response = self.client.get(
            reverse('shop:category-list', args=[self.category.slug]))
        self.assertTemplateUsed(response, 'shop/category-list.html')
